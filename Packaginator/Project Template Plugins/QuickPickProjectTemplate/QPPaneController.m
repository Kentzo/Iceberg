...
@implementation QPPaneController
...
- (IBAction) add:(id) sender
{
    ...
    [tOpenPanel beginSheetForDirectory:nil
                                  file:nil
                                 types:[NSArray arrayWithObject:@"quickpick"]
...
- (NSDragOperation)tableView:(NSTableView*)tv validateDrop:(id <NSDraggingInfo>)info proposedRow:(int)row proposedDropOperation:(NSTableViewDropOperation)op
{
    if (op==NSTableViewDropAbove)
    {
        ...
        {
            // Check that these PreferencePanes are not already in the list
            ...
            if (tArray!=nil)
            {
               NSString * tDraggedPath;
                ...
                for(i=0;i<tCount;i++)
                {
                    ...
                    if ([[tDraggedPath pathExtension] isEqualToString:@"quickpick"]==NO)
                    {
                        break;
                    }
...
- (BOOL) checkPaneValuesWithEngine:(id) inEngine
{
    ...
    if (tCount==1)
    {
        // Only one screen saver, we can use it to suggest the project name
    
        [inEngine setProjectName:[NSString stringWithFormat:@"%@",[[[pathsArray_ objectAtIndex:0] lastPathComponent] stringByDeletingPathExtension]]];
    }
...
- (void) processWithEngine:(id) inEngine
{
    ...
    if (tCount>0)
    {
        ...
        // Look for the PreferencePanes location in /Library
        
        tDictionary=[PBProjectDictionaryManager fileObjectAtPath:@"/Library/Application Support/Backup/QuickPicks" forPackageProject:tProjectDictionary];
