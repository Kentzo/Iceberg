#import "ScreenSaverPaneController.h"
...
@implementation ScreenSaverPaneController
...
- (IBAction) add:(id) sender
{
    ...
    [tOpenPanel beginSheetForDirectory:nil
                                  file:nil
                                 types:[NSArray arrayWithObject:@"saver"]
...
- (NSDragOperation)tableView:(NSTableView*)tv validateDrop:(id <NSDraggingInfo>)info proposedRow:(int)row proposedDropOperation:(NSTableViewDropOperation)op
{
    if (op==NSTableViewDropAbove)
    {
        ...
        {
            // Check that these ScreenSavers are not already in the list
            ...
            if (tArray!=nil)
            {
                ...
                for(i=0;i<tCount;i++)
                {
                    ...
                    if ([[tDraggedPath pathExtension] isEqualToString:@"saver"]==NO)
                    {
                        break;
                    }
                    
                    if ([pathsArray_ containsObject:tDraggedPath]==YES)
                    {
                        break;
                    }
                }
...
- (void) processWithEngine:(id) inEngine
{
    ...
    if (tCount>0)
    {
        ...
        // Look for the Screen Savers location in /Library
        
        tDictionary=[PBProjectDictionaryManager fileObjectAtPath:@"/Library/Screen Savers" forPackageProject:tProjectDictionary];
